...
grok_metrics = {
    "company_name": "EdgeRunner AI",
    "valuation_clean": 60_000_000,              # Estimated at ~$60M post–Series A
    "total_funding_clean": 17_500_000,          # $5.5M seed + $12M Series A :contentReference[oaicite:1]{index=1}
    "current_employees": 29,                    # From PitchBook listing :contentReference[oaicite:2]{index=2}
    "founded": 2024,                            # From PitchBook, LinkedIn :contentReference[oaicite:3]{index=3}
    "employee_growth": 10,                      # Estimated increase from ~19 to ~29 in last year
    "funding_per_employee": 17_500_000 / 29,
    "valuation_per_employee": 60_000_000 / 29,
    "capital_efficiency": 60_000_000 / 17_500_000,
}
...
def growth_scatter_plot(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        name='EdgeRunner AI', text=['EdgeRunner AI'], textposition='top right', showlegend=True
    ))
...
def funding_per_employee_competitors_chart(df):
    ...
    # Ensure EdgeRunner AI is always present
    if not (competitors['company_name'] == grok_metrics["company_name"]).any():
        extra_row = pd.DataFrame([{
            'company_name': grok_metrics["company_name"],
    ...
    fig = px.bar(competitors, x='company_name', y='funding_per_employee',
                 title='Funding per Employee: Competitors Near EdgeRunner AI',
...
def valuation_per_employee_competitors_chart(df):
    ...
    fig = px.bar(competitors, x='company_name', y='valuation_per_employee',
                 title='Valuation per Employee: Competitors Near EdgeRunner AI',
...
def headcount_vs_valuation_competitors_chart(df):
    ...
    fig = px.scatter(
        ...
        color='Industry',
        hover_name='company_name',
        title='Current Employees vs Valuation ($): EdgeRunner AI and Closest Peers',
        labels={'current_employees': 'Employees', 'valuation_clean': 'Valuation ($)'}
    )

    # Highlight EdgeRunner AI as a unique marker with label
    fig.add_trace(go.Scatter(
    ...
    # Y-axis: from a little below zero to EdgeRunner AI's valuation + 3B
